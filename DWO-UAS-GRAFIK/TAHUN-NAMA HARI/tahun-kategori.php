<?php
	...
	$det_kat = $conn->query("SELECT t.tahun tahun, f.kategori as kategori, sum(fp.amount) as pendapatan_kat FROM time t JOIN fakta_pendapatan fp ON (t.time_id = fp.time_id) JOIN film f ON (f.film_id = fp.film_id) GROUP BY tahun, kategori");
	$i = 0;
...
Highcharts.chart('container', {
    ...
    title: {
        text: 'Nilai Penjualan (WH Sakila)'
    },
    subtitle: {
        text: 'Klik bar untuk melihat detil nilai penjualan pertahun berdasarkan kategori'
    },
   accessibility: {
		announceNewData: {
			enabled: true
